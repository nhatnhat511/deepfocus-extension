...
export default function AdminUsers() {
  ...
  return (
    <section className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 className="text-base font-semibold text-slate-900">Admin list</h2>
        {loading ? (
          <p className="mt-3 text-sm text-slate-600">Loading admins...</p>
        ) : admins.length ? (
          <div className="mt-4 space-y-3">
            {admins.map((admin) => (
              <div key={admin.user_id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 px-3 py-3">
                <div>
                  <p className="text-sm font-semibold text-slate-900">{admin.email || admin.user_id}</p>
                  <p className="text-xs text-slate-600">Role: {admin.role || "admin"}</p>
                </div>
                <button
                  type="button"
