        ...
        ? createPortal(
            <div className="md:hidden">
              <button
                ...
                className="fixed inset-0 z-[9998] bg-slate-900/30 backdrop-blur-[1px]"
              />
              <div className="fixed left-0 top-0 z-[9999] h-full w-[68%] max-w-xs bg-white shadow-2xl ring-1 ring-slate-200/70 isolate">
                <nav className="h-full w-full border-r border-slate-200 px-4 py-6">
                  ...
                  <div className="flex flex-col text-sm text-slate-700">
                    {navLinks.map((item) => (
                      <Link
                        key={item.href}
                        href={item.href}
                        className="rounded-md border-b border-slate-100 px-4 py-3.5 font-semibold transition-colors last:border-b-0 hover:bg-slate-50 hover:text-slate-900 active:bg-slate-100"
                        onClick={() => setMobileOpen(false)}
                      >
                        {item.label}
                      </Link>
